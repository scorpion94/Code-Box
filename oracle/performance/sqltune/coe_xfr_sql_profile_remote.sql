SPO coe_xfr_sql_profile_remote.log;
...
REM
REM $Header: 215187.1 coe_xfr_sql_profile_remote.sql 19.7 2021/07/16 stelios.charalambides $
REM
...
REM
REM SCRIPT
REM   coe_xfr_sql_profile_remote.sql
REM
REM DESCRIPTION
...
REM   It also warns of remote components and optionally aborts if
REM   this is the case. 
REM
REM PRE-REQUISITES
...
REM   3. TRUE or FALSE or YES or NO to control abort on remote component.
REM
REM EXECUTION
REM   1. Connect into SQL*Plus as user with access to data dictionary.
REM      Do not use SYS.
REM   2. Execute script coe_xfr_sql_profile_remote.sql passing SQL_ID and
REM      plan hash value (parameters can be passed inline or until
REM      requested) and a parameter for abort on remote.
REM
REM EXAMPLE
REM   # sqlplus system
REM   SQL> START coe_xfr_sql_profile.sql [SQL_ID] [PLAN_HASH_VALUE] [ABORT_ON_REMOTE];
REM   SQL> START coe_xfr_sql_profile.sql gnjy0mn4y9pbm 2055843663 TRUE;
REM   SQL> START coe_xfr_sql_profile.sql gnjy0mn4y9pbm FALSE;
REM   SQL> START coe_xfr_sql_profile.sql;
REM
REM NOTES
REM   1. For possible errors see coe_xfr_sql_profile_remote.log
...
PRO
PRO Parameter 3:
PRO ABORT_ON_REMOTE (required)
PRO
DEF abort_on_remote = '&3';
PRO
PRO Values passed to coe_xfr_sql_profile:
PRO ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PRO SQL_ID         : "&&sql_id."
PRO PLAN_HASH_VALUE: "&&plan_hash_value."
PRO ABORT_ON_REMOTE: "&&abort_on_remote."
PRO
variable remote_qblock_count number
variable abort_on_remote varchar2(5)
execute :abort_on_remote := '&&abort_on_remote';
WHENEVER SQLERROR EXIT SQL.SQLCODE;
declare
begin
  IF ( upper(:abort_on_remote) <> 'NO' or :abort_on_remote is null) THEN 
    :abort_on_remote := 'TRUE' ;
  END IF;
  select count(1) into :remote_qblock_count from v$sql_plan where PLAN_HASH_VALUE=&&plan_hash_value and operation='REMOTE';
  IF (:remote_qblock_count > 0 AND upper(:abort_on_remote) = 'TRUE' ) THEN
    RAISE_APPLICATION_ERROR(-20000, 'SQL_ID : &&sql_id PHV : &&plan_hash_value has a REMOTE component. Aborting');
  END IF;
END;
/
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE('REM   stelios.charalambides@oracle.com');
  DBMS_OUTPUT.PUT_LINE('REM');
  ...
  DBMS_OUTPUT.PUT_LINE('REM   This script is generated by coe_xfr_sql_profile_remote.sql');
  ...
  IF :remote_qblock_count > 0 then 
    DBMS_OUTPUT.PUT_LINE('PRO COE_XFR_SQL_PROFILE_REMOTE: WARNING: REMOTE component(s) found in this PHV');
  END IF;
END;
/
SPO OFF;
...
PRO
UNDEFINE 1 2 sql_id plan_hash_value remote_qblock_count abort_on_remote
CL COL
PRO
PRO COE_XFR_SQL_PROFILE_REMOTE completed.
